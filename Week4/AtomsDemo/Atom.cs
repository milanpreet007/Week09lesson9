...
namespace AtomsDemo
{
    class Atom
    {
        ...
        public static Atom Parse(string line)
        {
            ...
            string name = parts[0];
            int proton = Convert.ToInt32(parts[1]);
            int neutron = Convert.ToInt32(parts[2]);
            double weight= Convert.ToDouble(parts[3]);
            string symbol = parts[4];
            return new Atom(name,proton,neutron,weight,symbol);
        }
        public override string ToString() => $"{Proton,4}{Name} ({Symbol}) {Neutron} {Weight:f4}";
